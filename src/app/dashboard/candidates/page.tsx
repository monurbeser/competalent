...
import { Loader2, UploadCloud, MapPin, Briefcase, Trash2, Edit2, Phone, Mail, FileText, Sparkles, Save, RefreshCw } from "lucide-react";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        {loading && <div className="p-8 text-center text-slate-500">Loading candidates...</div>}
        {!loading && candidates.length === 0 && <div className="p-12 text-center text-slate-400">No candidates found.</div>}
        ...
        <div className="divide-y divide-slate-100">
          {candidates.map((candidate) => (
            <div 
              ...
              className="group grid grid-cols-12 gap-4 p-4 items-center hover:bg-blue-50/50 transition-all cursor-pointer relative"
            >
              <div className="col-span-4 flex items-center gap-3 pl-2">
                ...
                <div>
                  <p className="font-semibold text-slate-800 group-hover:text-blue-700">{candidate.full_name || "Unknown"}</p>
                  <p className="text-xs text-slate-400">{candidate.email}</p>
                </div>
              </div>
              <div className="col-span-3 text-sm text-slate-600 truncate" title={candidate.summary}>
                {candidate.summary || "No summary"}
              </div>
              <div className="col-span-3 flex items-center gap-2 text-sm text-slate-600">
                <MapPin size={14} className="text-slate-400" />
                {candidate.parsed_data?.location || "Unknown"}
              </div>
              <div className="col-span-2 flex justify-end gap-2 pr-2">
                <Button variant="ghost" size="icon" onClick={(e) => openEditProfile(e, candidate)} className="text-slate-400 hover:text-blue-600">
                    <Edit2 size={16} />
                </Button>
                <Button variant="ghost" size="icon" onClick={(e) => handleDelete(e, candidate.id, candidate.resume_url)} className="text-slate-400 hover:text-red-600">
                    <Trash2 size={16} />
                </Button>
              </div>
            </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        {/* !max-w-[95vw] ile kısıtlamayı eziyoruz */}
                    ...
                    <div className="flex justify-between items-start relative z-10">
                        <div className="flex gap-6 items-center">
                            ...
                            <div className="space-y-2">
                                {isEditMode ? (
                                    <Input 
                                        className="bg-white/10 border-white/20 text-white font-bold text-2xl h-12 w-96" 
                                ...
                                <div className="flex flex-wrap gap-6 text-sm text-slate-300 font-medium">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <Mail size={16}/> 
                                        {isEditMode ? (
                                          <Input className="h-7 w-56 bg-white/10 border-white/20 text-white text-xs" value={formData.email || ""} onChange={e=>setFormData({...formData, email: e.target.value})}/>
                                        ) : selectedCandidate.email}
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Phone size={16}/>
                                        {isEditMode ? (
                                          <Input className="h-7 w-40 bg-white/10 border-white/20 text-white text-xs" value={formData.phone || ""} onChange={e=>setFormData({...formData, phone: e.target.value})}/>
                        ...
                        <div className="flex gap-3">
                             <Button 
                                size="sm" 
                                className="bg-indigo-600 hover:bg-indigo-500 text-white border border-indigo-400"
                             ...
                             {isEditMode && (
                                 <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white" onClick={handleSaveChanges}>
                                     <Save size={14} className="mr-2" /> Save
                                 </Button>
                             )}
                             <Button size="sm" variant="outline" className="bg-white/10 border-white/20 text-white hover:bg-white/20" onClick={() => window.open(selectedCandidate.resume_url, '_blank')}>
                                 <FileText size={14} className="mr-2" /> CV
                             </Button>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="col-span-12 lg:col-span-4 space-y-6">
                            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                <h4 className="font-bold text-slate-800 mb-4 flex items-center gap-2 text-lg">
                                    <Briefcase size={20} className="text-blue-600"/> Summary
                                </h4>
                                {isEditMode ? (
                                    <Textarea 
                                        className="text-sm text-slate-600 min-h-[150px] leading-relaxed" 
                            ...
                            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                                <h4 className="font-bold text-slate-800 mb-4 flex items-center gap-2 text-lg">
                                    <Sparkles size={20} className="text-purple-600"/> Skills
                                </h4>
                                <div className="flex flex-wrap gap-2">
                                    {selectedCandidate.skills && selectedCandidate.skills.length > 0 ? (
                                        ...
                                        <span className="text-sm text-slate-400 italic">No skills parsed yet. Click 'Re-Analyze'.</span>
                                    )}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="col-span-12 lg:col-span-8 space-y-6">
                            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-100 shadow-sm">
                                <h4 className="font-bold text-blue-900 mb-3 flex items-center gap-2 text-lg">
                                    <Sparkles size={20} className="text-blue-600"/> Competalent AI Insight
                                </h4>
                                {isEditMode ? (
                                    <Textarea 
                                        className="bg-white/80 border-blue-200 min-h-[80px]" 
                                    ...
                                    <p className="text-base text-blue-800 italic leading-relaxed">
                                        "{selectedCandidate.ai_analysis || "No AI analysis generated yet."}"
                                    </p>
                                )}
                            </div>
                                ...
                                <div className="space-y-8">
                                    {selectedCandidate.experience && selectedCandidate.experience.length > 0 ? (
                                        selectedCandidate.experience.map((exp, i) => (
                                            <div key={i} className="flex gap-6 relative group">
                                                {i !== selectedCandidate.experience.length - 1 && (
                                                    <div className="absolute left-[20px] top-10 bottom-[-32px] w-0.5 bg-slate-200 group-hover:bg-blue-200 transition-colors"></div>
                                                )}
                                                <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shrink-0 border-2 border-slate-200 z-10 shadow-sm group-hover:border-blue-400 transition-colors">
                                                    <Briefcase size={18} className="text-slate-400 group-hover:text-blue-600" />
                                                </div>
                                                <div className="flex-1">
                                                    <h5 className="font-bold text-slate-800 text-lg">{exp.title}</h5>
                                                    <div className="text-base text-slate-600 font-medium mb-1">{exp.company}</div>
                                                    <span className="text-xs font-semibold text-slate-500 bg-slate-100 px-3 py-1 rounded-full inline-block">
                                                        {exp.years}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <div className="text-center py-12 text-slate-400">
                                            ...
                                            <p className="text-lg">No experience record found.</p>
                                            <Button variant="link" onClick={handleReAnalyze} className="text-indigo-600">Try Re-Analyze</Button>
                                        </div>
                                    )}
                                </div>
                            </div>
